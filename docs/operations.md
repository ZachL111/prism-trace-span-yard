...
`prism-trace-span-yard` is centered on observability work in Scala. The technical target is: Package a Scala local lab for span analysis with windowed input fixtures, late-data behavior checks, and documented operating limits.
...
The score is deterministic. The current threshold is 157. Risk has a penalty of 4, latency has a penalty of 3, and weight contributes a bonus of 4.
